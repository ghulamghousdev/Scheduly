...
class AddTeacher extends React.Component {
    ...
    handleSelect(e){
        ...
        this.setState((prevState)=>{
            return {selectedSubjects: prevState.selectedSubjects.concat(subject[0])}
        });
    }
        ...
        return(
            <div className="addform addteacher">
                <form>
                    ...
                    {this.state.selectedSubjects.map((cur)=> (<p className="addform__selectedOptions">{`${cur.subjectCode} | ${cur.subjectName}`}</p>))}
                    <div className="addform__dropdown">
                        <select name="subjects" id="subjects" className="addform__select">
                        {
                            this.state.subjects.map((cur)=> (<option className="addform__option" value={cur.subjectCode}>{`${cur.subjectCode} | ${cur.subjectName}`}</option>))
                        }
                        </select>
                        <input onClick={this.handleSelect} className="addform__btn" type="button" value="SELECT"/>
                    </div>
                    <input 
